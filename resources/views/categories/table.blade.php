<div class="col-md-4 col-sm-4">
    @if (count($checked) >= 1)
        ...
        (<b>{{count($checked)}}</b> Selected Item To<b>ERASE </b>)
        </a>
    @endif
</div>
    ...
    <tbody>
        @forelse ($sections as $section)
            <tr class="">
                ...
                <td>{{ $section->status == 1 ? 'Active' : 'Deactivated' }}</td>
                <td>
                    <div class="btn-group">
                        <a href="#editSection" data-toggle="modal" wire:click.prevent="editSection({{ $section->id }})" 
                            class="btn btn-outline-info btn-rounder"><i class="fa fa-edit"></i></a>
                        @if (count($checked) <2)
                        <a href="#" wire:click.prevent="ConfirmDelete({{ $section->id }}, '{{ $section->section_name }}')" 
                            class="btn btn-outline-danger btn-rounder"><i class="fa fa-trash"></i></a>
                        @endif
                    </div>
                </td>
            </tr>
